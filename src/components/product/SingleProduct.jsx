...
const SingleProduct = ({ glass }) => {
    const { id, image, name, rating, price, description } = glass;

    return (
        <section key={id} className="flex flex-col justify-between bg-white/[0.5] rounded-sd
        ...
        ">
            ...
            <main className="p-3 gap-2 mt-2 flex flex-row">
                ...
                <aside className="flex flex-col items-end">
                    <p className="flex justify-between">
                        <h5 className="text-amber-400">₹{price}</h5>
                        {/* <span className="text-gray-500 px-1">(incl.tax)</span> */}
                    </p>
                    <h6 className="text-sm text-gray-400 line-through">₹{price + 100}</h6>
                </aside>
            </main>
            ...
            <div className="px-3 py-2 border-t justify-between flex items-center w-full">
                <button className="border border-[--primary-text-color]  
